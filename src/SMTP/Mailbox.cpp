...
Mailbox::Mailbox(const std::string& address) 
	: m_user{TryParseUser(address)}, m_host{TryParseHost(address)} 
	, m_address{address}
{
}

std::optional<std::string> Mailbox::GetUser() const noexcept
{
	return m_user;
}

std::optional<std::string> Mailbox::GetHost() const noexcept
{
	return m_host;
}

std::string Mailbox::GetFullAddress() const noexcept
{
    return m_address;
}
