...
ServerBase::ServerBase(std::shared_ptr<asio::io_context> io_context, std::shared_ptr<asio::ssl::context> ssl_context,
					   const Port port) :
	m_io_context{io_context}, m_ssl_context{ssl_context}, m_started{false}, m_acceptor{*m_io_context},
	m_endpoint{asio::ip::tcp::v4(), port}, m_session_register{std::make_shared<SessionRegisterBase>()}
{
}
