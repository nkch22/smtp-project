...
HeloCommand::HeloCommand(const std::string client_domain) : m_client_domain{std::move(client_domain)} {}

Response HeloCommand::Execute(Context& options)
{
	const Response response{ReplyCode::Ok, std::format("{} greets {}", options.domain_name, m_client_domain)};
	return response;
}

OptionalCommand HeloCommand::TryParseCommand(const std::string& request, const Context& options)
{
	if (const auto position{request.find(HeloCommand::COMMAND)}; position != std::string::npos)
	{
